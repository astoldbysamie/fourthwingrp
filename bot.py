...
INFANTRY_GROUP_LORE = {
    "First Division": ("Frontline", "Vanguard or Bastion"),
    "Second Division": ("Mobility", "Skirmisher or Ranger"),
    "Third Division": ("Force", "Breaker"),
    "Fourth Division": ("Command", "Tactician"),
}
SCRIBE_GROUP_LORE = {
    "First Order": ("Records", "Archive or Chronicle"),
    "Second Order": ("Lexicon", "Lexicon"),
    "Third Order": ("Intelligence", "Intelligence or Ciphers and Decoding"),
    "Fourth Order": ("Restricted", "Restricted"),
}
HEALER_GROUP_LORE = {
    "First Circle": ("Field Operations", "Battlefield or Emergency"),
    "Second Circle": ("Precision", "Surgical or Recovery"),
    "Third Circle": ("Experimental", "Experimental"),
    "Fourth Circle": ("Dragonkin", "Dragonkind"),
}

def format_group_lore(group_name: str, quadrant: str) -> str:
    maps = {"infantry": INFANTRY_GROUP_LORE, "scribe": SCRIBE_GROUP_LORE, "healer": HEALER_GROUP_LORE}
    lore = maps.get(quadrant, {}).get(group_name)
    if not lore:
        return ""
    focus, specialty = lore
    return f"{group_name}. {focus}. Specialty: {specialty}"

def format_simple_assignment_with_lore(name: str, slot, quadrant: str) -> str:
    role, group_name = slot
    base = f"**{name}** assigned as **{role}**."
    if group_name == "_chain":
        return base + "\nHigh Chain assignment."
    lore = format_group_lore(group_name, quadrant)
    return base + (f"\n{lore}" if lore else "")
...
async def call_command(command_or_func, interaction: discord.Interaction, *args):
    """Run a slash command callback safely from a button or modal."""
    callback = getattr(command_or_func, "callback", command_or_func)
...
async def slash_random(interaction: discord.Interaction, kind: app_commands.Choice[str]):
    if kind.value == "threshing":
        await call_command(slash_threshing, interaction)
    elif kind.value == "signet":
        await call_command(slash_signet, interaction)
    elif kind.value == "dragon_speak":
        await call_command(slash_dragonspeak, interaction)
    elif kind.value == "dragon_action":
        await call_command(slash_dragonaction, interaction)
    elif kind.value == "infantry":
        await call_command(slash_infantry, interaction)
    elif kind.value == "scribe":
        await call_command(slash_scribe, interaction)
    else:
        await call_command(slash_healer, interaction)
...
async def slash_assigninfantry(interaction: discord.Interaction, name: str):
    ...
    assign_simple_slot(infantry_data, name, slot, "Cadet", "Cadets")
    save_json_file(INFANTRY_FILE, infantry_data)
    await interaction.response.send_message(format_simple_assignment_with_lore(name, slot, "infantry"))
...
async def slash_manualinfantry(interaction: discord.Interaction, name: str, role: app_commands.Choice[str], division: app_commands.Choice[str] = None):
    ...
    if error:
        ...
        return
    save_json_file(INFANTRY_FILE, infantry_data)
    await interaction.response.send_message(f"⚔️ **{name}** manually assigned as **{role.value}**." + (f"\n{format_group_lore(division.value, 'infantry')}" if division else ""))
...
async def slash_assignscribe(interaction: discord.Interaction, name: str):
    ...
    assign_simple_slot(scribe_data, name, slot, "Scribe", "Scribes")
    save_json_file(SCRIBE_FILE, scribe_data)
    await interaction.response.send_message(format_simple_assignment_with_lore(name, slot, "scribe"))
...
async def slash_manualscribe(interaction: discord.Interaction, name: str, role: app_commands.Choice[str], order: app_commands.Choice[str] = None):
    ...
    if error:
        ...
        return
    save_json_file(SCRIBE_FILE, scribe_data)
    await interaction.response.send_message(f"📚 **{name}** manually assigned as **{role.value}**." + (f"\n{format_group_lore(order.value, 'scribe')}" if order else ""))
...
async def slash_assignhealer(interaction: discord.Interaction, name: str):
    ...
    assign_simple_slot(healer_data, name, slot, "Trainee", "Trainees")
    save_json_file(HEALER_FILE, healer_data)
    await interaction.response.send_message(format_simple_assignment_with_lore(name, slot, "healer"))
...
class SafeView(discord.ui.View):
    async def on_error(self, interaction: discord.Interaction, error: Exception, item) -> None:
        print(f"Button error in {type(item).__name__}: {error}")
        message = f"That button hit an error: `{type(error).__name__}: {error}`"
        try:
            ...
            print(f"Could not send button error message: {send_error}")

class SafeModal(discord.ui.Modal):
    async def on_error(self, interaction: discord.Interaction, error: Exception) -> None:
        print(f"Modal error in {type(self).__name__}: {error}")
        message = f"That form hit an error: `{type(error).__name__}: {error}`"
        try:
            if interaction.response.is_done():
                await interaction.followup.send(message, ephemeral=True)
            else:
            ...
            print(f"Could not send modal error message: {send_error}")
...
class NameModal(SafeModal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        await call_command(self.callback_func, interaction, str(self.name_input.value).strip())


class TwoNameModal(SafeModal):
...
class DiceModal(SafeModal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        await call_command(slash_roll, interaction, str(self.dice.value).strip())
...
class ConfirmResetView(SafeView):
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        await call_command(self.callback_func, interaction)
...
class ManualNameModal(SafeModal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if self.quadrant == "rider":
            await call_command(slash_manualassign, 
                interaction,
                name,
            ...
            await call_command(slash_manualinfantry, 
                interaction,
                name,
                app_commands.Choice(name=self.selections.get("role"), value=self.selections.get("role")),
            ...
            await call_command(slash_manualscribe, 
                interaction,
                name,
                app_commands.Choice(name=self.selections.get("role"), value=self.selections.get("role")),
            ...
            await call_command(slash_manualhealer, 
                interaction,
                name,
                app_commands.Choice(name=self.selections.get("role"), value=self.selections.get("role")),
...
class CharacterRandomizeView(SafeView):
    ...
    async def randomize(self, interaction: discord.Interaction, quadrant_value: str | None):
        ...
        await call_command(slash_createcharacter, interaction, choice)
...
class RosterLookupPanelView(SafeView):
    def __init__(self): super().__init__(timeout=240)
    @discord.ui.button(label="View Full Roster", style=discord.ButtonStyle.primary)
    async def roster(self, interaction, button): await call_command(slash_roster, interaction, app_commands.Choice(name="All", value="all"))
    @discord.ui.button(label="Names Only", style=discord.ButtonStyle.secondary)
    ...
    async def infantryslots(self, interaction, button): await call_command(slash_infantryslots, interaction)
    @discord.ui.button(label="Scribe Formation", style=discord.ButtonStyle.secondary)
    async def scribeslots(self, interaction, button): await call_command(slash_scribeslots, interaction)
    @discord.ui.button(label="Healer Formation", style=discord.ButtonStyle.secondary)
    async def healerslots(self, interaction, button): await call_command(slash_healerslots, interaction)

class ManualAssignCharacterPanelView(SafeView):
    def __init__(self): super().__init__(timeout=240)
    @discord.ui.button(label="Manual Assign Rider", style=discord.ButtonStyle.primary)
    async def rider(self, interaction, button): await interaction.response.send_message("**Manual Assign Rider**\nChoose role, wing, section, and squad if needed.", view=ManualAssignView("rider"), ephemeral=True)
    @discord.ui.button(label="Manual Assign Infantry", style=discord.ButtonStyle.primary)
    async def infantry(self, interaction, button): await interaction.response.send_message("**Manual Assign Infantry**\nChoose role and division if needed.", view=ManualAssignView("infantry"), ephemeral=True)
    @discord.ui.button(label="Manual Assign Scribe", style=discord.ButtonStyle.primary)
...
class SpecialtyDragonPanelView(SafeView):
    def __init__(self): super().__init__(timeout=240)
    @discord.ui.button(label="Infantry Specialty", style=discord.ButtonStyle.primary)
    async def infantry(self, interaction, button): await call_command(slash_infantry, interaction)
    @discord.ui.button(label="Scribe Specialty", style=discord.ButtonStyle.primary)
    async def scribe(self, interaction, button): await call_command(slash_scribe, interaction)
    @discord.ui.button(label="Healer Discipline", style=discord.ButtonStyle.primary)
    async def healer(self, interaction, button): await call_command(slash_healer, interaction)
    @discord.ui.button(label="Dragon Color + Tail", style=discord.ButtonStyle.primary)
    async def threshing(self, interaction, button): await call_command(slash_threshing, interaction)

class CombatPanelView(SafeView):
    def __init__(self): super().__init__(timeout=240)
    ...
    @discord.ui.button(label="Full RP Fight", style=discord.ButtonStyle.success)
    ...
    async def masterboard(self, interaction, button): await call_command(slash_masterboard, interaction)
...
class MatChallengesPanelView(SafeView):
    def __init__(self): super().__init__(timeout=240)
    @discord.ui.button(label="Active Mats", style=discord.ButtonStyle.success)
    async def active(self, interaction, button): await call_command(slash_activemats, interaction)
    @discord.ui.button(label="Random Mat Pairs", style=discord.ButtonStyle.success)
    async def pairs(self, interaction, button): await call_command(slash_matpairs, interaction)

class GauntletPanelView(SafeView):
    def __init__(self): super().__init__(timeout=240)
    @discord.ui.button(label="Run Gauntlet", style=discord.ButtonStyle.success)
    ...
    async def hazard(self, interaction, button): await call_command(slash_gauntlethazard, interaction)
    ...
    async def injury(self, interaction, button): await call_command(slash_gauntletinjury, interaction)
    @discord.ui.button(label="Gauntlet Outcome", style=discord.ButtonStyle.secondary)
    async def outcome(self, interaction, button): await call_command(slash_gauntletoutcome, interaction)

class DragonReactionsPanelView(SafeView):
    def __init__(self): super().__init__(timeout=240)
    @discord.ui.button(label="Dragon Speak", style=discord.ButtonStyle.success)
    async def ds(self, interaction, button): await call_command(slash_dragonspeak, interaction)
    @discord.ui.button(label="Dragon Action", style=discord.ButtonStyle.success)
    async def da(self, interaction, button): await call_command(slash_dragonaction, interaction)

class DicePanelView(SafeView):
    def __init__(self): super().__init__(timeout=240)
    @discord.ui.button(label="Roll Custom Dice", style=discord.ButtonStyle.success)
    ...
    async def d4(self, interaction, button): await call_command(slash_die, interaction, app_commands.Choice(name="d4", value="d4"))
    @discord.ui.button(label="d6", style=discord.ButtonStyle.secondary)
    async def d6(self, interaction, button): await call_command(slash_die, interaction, app_commands.Choice(name="d6", value="d6"))
    @discord.ui.button(label="d8", style=discord.ButtonStyle.secondary)
    async def d8(self, interaction, button): await call_command(slash_die, interaction, app_commands.Choice(name="d8", value="d8"))
    @discord.ui.button(label="d10", style=discord.ButtonStyle.secondary)
    async def d10(self, interaction, button): await call_command(slash_die, interaction, app_commands.Choice(name="d10", value="d10"))
    @discord.ui.button(label="d12", style=discord.ButtonStyle.secondary)
    async def d12(self, interaction, button): await call_command(slash_die, interaction, app_commands.Choice(name="d12", value="d12"))
    @discord.ui.button(label="d20", style=discord.ButtonStyle.secondary)
    async def d20(self, interaction, button): await call_command(slash_die, interaction, app_commands.Choice(name="d20", value="d20"))
    @discord.ui.button(label="d100", style=discord.ButtonStyle.secondary)
    async def d100(self, interaction, button): await call_command(slash_die, interaction, app_commands.Choice(name="d100", value="d100"))

class DeleteCharacterPanelView(SafeView):
    def __init__(self): super().__init__(timeout=240)
    @discord.ui.button(label="Remove Rider", style=discord.ButtonStyle.danger)
    ...
    @discord.ui.button(label="Remove Infantry", style=discord.ButtonStyle.danger)
    ...
    @discord.ui.button(label="Remove Scribe", style=discord.ButtonStyle.danger)
